...
class MptAttention(nn.Module):
    ...
    def forward(
        ...
        mask_map = None # added by HSY 1/26/24
    ):
        #added by HSY 10/7/23 ---
        # for debugging purposes. mask_map should never be None
        if mask_map == None: 
            ipdb.set_trace()
        #----------------------
        ...
        if mask_map is not None:
            mask_map = mask_map.view(mask_map.size(0), 1, mask_map.size(1), mask_map.size(1)).int()
            mask_map = (1.0 - mask_map) * torch.finfo(attention_scores.dtype).min
            attention_scores = attention_scores + mask_map
        # ---------------------------
...
class MptBlock(nn.Module):
    ...
    def forward(
        ...
        attn_outputs, attn_weights, past_key_value = self.attn(
            ...
            mask_map=mask_map, # added by HSY 1/26/24
        )
...
class MptModel(MptPreTrainedModel):
    ...
    def forward(
        ...
        mask_map = None, # added by HSY 1/26/24
        mask_image_map = None, # added by HSY 1/26/24
        ...
        for i, (block, layer_past) in enumerate(zip(self.blocks, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                outputs = block(
                    ...
                    position_bias=alibi,
                    mask_map=mask_map, # added by HSY 1/26/24
                    mask_image_map=mask_image_map, # added by HSY 1/26/24
                )
