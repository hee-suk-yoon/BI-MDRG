...
from custom_files.custom_helpers import PerceiverResampler
...
from custom_files.custom_utils import apply_with_stopping_condition 
...
class Flamingo(nn.Module):
    ...
    def forward(
        ...
        mask_image_map: torch.Tensor = None,
        ...
        """
        if sum(image_map) != vision_x.shape[0]:
            import ipdb; ipdb.set_trace()
            
        assert sum(image_map) == vision_x.shape[0], "image_map is not correct"
        #----------------------
        ...
        if self.lang_encoder._use_cached_vision_x:
            ...
            self._encode_vision_x(vision_x=vision_x, image_map=image_map) 
    ...
    def _encode_vision_x(self, vision_x: torch.Tensor,
        ...
        with torch.no_grad():
            if sum(image_map) != 0:
                ...
                #this is the case where there is no image in the batch.
                temp_vision_x_device = vision_x.device
                vision_x = None
        ...
        # preprocess the vision_x for our need.
        if image_map is None:
            # for debugging purpose.
            import ipdb; ipdb.set_trace() 
        else:
            def divide_into_batches(images, num_images_per_batch):
                # Compute the maximum number of images in a batch
                max_images = max(num_images_per_batch)
                
                # Divide the images into batches
                batched_images_list = []
                start_idx = 0
                for num in num_images_per_batch:
                    end_idx = start_idx + num
                    batch = images[start_idx:end_idx].squeeze(1)
                    
                    # Pad the batch if necessary
                    if num < max_images:
                        padding_size = max_images - num
                        pad_tensor = torch.zeros((padding_size, 64, 1024), dtype=images.dtype, device=images.device)
                        batch = torch.cat([batch, pad_tensor], dim=0)
                    
                    batched_images_list.append(batch)
                    start_idx = end_idx
                
                # Combine all the batches into a single tensor
                result = torch.stack(batched_images_list, dim=0)
                
                return result
            if vision_x is not None:
                vision_x = divide_into_batches(vision_x, image_map)
            else:
                vision_x = torch.zeros(len(image_map), 1, 64, 1024, dtype=torch.float32, device=temp_vision_x_device)
