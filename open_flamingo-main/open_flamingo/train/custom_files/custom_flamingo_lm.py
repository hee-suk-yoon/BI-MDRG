...
from custom_files.custom_helpers import GatedCrossAttentionBlock

#from .utils import getattr_recursive, setattr_recursive
from custom_files.custom_utils import getattr_recursive, setattr_recursive
        ...
        **decoder_layer_kwargs,
        ...
        if self.gated_cross_attn_layer is not None:
            ...
            lang_x = self.gated_cross_attn_layer(
                ...
                custom_text_to_image_mask=mask_image_map,
            )

        # Normal decoder layer
        lang_x = self.decoder_layer(
            lang_x, attention_mask=attention_mask, mask_map=mask_map, **decoder_layer_kwargs
        )
        return lang_x
...
class FlamingoLMMixin(nn.Module):
    ...
    #added by Hee Suk Yoon 10/7/23
    def _encode_special_tokens(self):
        flamingo_layers = self._get_decoder_layers()
        for flamingo_layer in flamingo_layers:
            flamingo_layer.media_token_id = self.media_token_id
            flamingo_layer.eoc_token_id = self.eoc_token_id
            flamingo_layer.media_end_token_id = self.media_end_token_id
        return

    def forward(self, input_ids, attention_mask, mask_map, mask_image_map, **kwargs):
        ...
        kwargs["mask_image_map"] = mask_image_map
