...
def main():
    ...
    parser.add_argument(
        '--full_tuning', #whether to use the visual cross attention module.
        action='store_true',
    )

    parser.add_argument(
        "--resume_after_pretraining",
        type=str,
        help="path to checkpoint to resume from, this should contain model, optimizer, and lr_scheduler states. if there exists a checkpoint in the dir named run_name, we will resume from that checkpoint by default",
        default=None,
    )
    ...
    if args.resume_after_pretraining is not None:
        if args.rank == 0:
            print(f"Loading checkpoint from {args.resume_after_pretraining}")
        checkpoint = torch.load(args.resume_after_pretraining, map_location="cpu")
        try:
            msd = checkpoint["model_state_dict"]
            msd = {k.replace("module.", ""): v for k, v in msd.items()}
            resume_from_epoch = checkpoint["epoch"] + 1
